...
function partition_indirect(triangleBuffer, triangleBounds, offset, count, split) {
	...
	while ( true ) {
		...
		if (left < right) {
			...
			// for (let i = 0; i < 3; i++) {
			// 	let t1 = triangleBounds[ left * 6 + i * 2 + 0 ]; // coordinate center for left tri
			// 	triangleBounds[ left * 6 + i * 2 + 0 ] = triangleBounds[ right * 6 + i * 2 + 0 ];
			// 	triangleBounds[ right * 6 + i * 2 + 0 ] = t1;

			// 	let t2 = triangleBounds[ left * 6 + i * 2 + 1 ]; // coordinate delta for left tri
...
function buildTree( geo, options, indirectTriangleBuffer = null ) {
	...
	function splitNode( node, offset, count, centroidBoundingData = null, depth = 0 ) {
		...
		if (indirectTriangleBuffer != null) {
			splitOffset = partition_indirect(indirectTriangleBuffer, triangleBounds, offset, count, split);
		} else {
	...
	// [RM] removing reference to base indices
...
function buildPackedTree( geo, options, indirectTriangleBuffer = null ) {

	// boundingData  				: 6 float32
	// right / offset 				: 1 uint32
	// splitAxis / isLeaf + count 	: 1 uint32 / 2 uint16

	// [RM] pass indirectTriangleBuffer to buildTree:
...
function intersectTri( geo, side, ray, tri, intersections ) {
	// [RM] update intersection index getter to account for indirect triangle buffer
	...
	if (bvh.indirectTriangleBuffer != null) {
		const itb = bvh.indirectTriangleBuffer;
		const mappedTriOffset = itb[triOffset];
...
export { AVERAGE, CENTER, CONTAINED, INTERSECTED, MeshBVH, MeshBVHVisualizer, NOT_INTERSECTED, SAH, acceleratedRaycast, computeBoundsTree, disposeBoundsTree, estimateMemoryInBytes, getBVHExtremes, getJSONStructure, validateBounds };
